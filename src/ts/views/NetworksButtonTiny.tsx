...
export default class NetworksButtonTiny extends ComponentBase<AccountMenuButtonProps, AccountMenuButtonState> {
    ...
    render(): JSX.Element | null {
        return (
            ...
            <UI.Button ref={this._onMountButton} onPress={this._onPress} iconSlot={iconStyle => (
                <RX.Image style={{ width: 17, height: 17 }} source={this.props.network === "Polygon" ? ImageSource.matic : this.props.network === "Fantom" ? ImageSource.fantom : this.props.network === "Binance" ? ImageSource.bnb : this.props.network === "Avalanche" ? ImageSource.avax : ""} />
            )} style={{ content: [{ width: 46, height: 37, justifyContent: 'center', alignItems: 'center', borderRadius: 11, }], label: _styles.label }
    ...
    private _onSelectMenuItem = async (command: string) => {
        ...
        switch (command) {
            case 'onPolygon':
                this.props.onPolygon()
                return
            case 'onAvalanche':
                this.props.onAvalanche()
                return
            case 'onBinance':
                this.props.onBinance()
                return
            case 'onAvalanche':
                this.props.onAvalanche()
